...
copilot_appid=`linera --with-wallet 0 create-application $copilot_bid --json-parameters "{\"review_app_id\":\"$review_appid\",\"foundation_app_id\":\"$foundation_appid\",\"feed_app_id\":\"$feed_appid\"}" --required-application-ids $review_appid --required-application-ids $foundation_appid --required-application-ids $feed_appid`
...
sed -i "s/blobGatewayApp =.*/blobGatewayApp = '$blobGateway_appid'/g" webui/src/const/index.ts
